...
export default function GitHubActivity() {
    ...
    if (error) {
        return (
            <div className="w-full max-w-4xl mx-auto mt-8 p-6 bg-black/30 backdrop-blur-sm rounded-lg">
                <p className="text-red-400">Unable to load GitHub activity: {error}</p>
                <p className="text-gray-400 mt-2">Please check if the API route is working and try again later.</p>
            </div>
        );
    }
    ...
    return (
        <div className="w-full max-w-4xl mx-auto mt-8 p-6 bg-black/30 backdrop-blur-sm rounded-lg pt-20">
            <div className="overflow-x-auto">
                <div className="inline-block">
                    <div className="grid" style={{ gridTemplateColumns: `repeat(${numCols}, minmax(0, 1fr))`, gridTemplateRows: `repeat(7, minmax(0, 1fr))` }}>
                        {grid.map((row, rowIdx) =>
                            row.map((day, colIdx) => (
                                <div
                                    key={`${rowIdx}-${colIdx}`}
